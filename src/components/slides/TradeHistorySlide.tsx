...
import { UnfairTradeRecord, WrappedData } from "@/lib/types";
...
function formatTradeLabel(trade: UnfairTradeRecord) {
  if (trade.tradeDate) {
    return trade.week ? `${trade.tradeDate} · Week ${trade.week}` : trade.tradeDate;
  }
...
export function TradeHistorySlide({ data, direction, trade, index, total }: Props) {
  return (
